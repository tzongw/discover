...
_SCRIPT = """#!lua name=zset_timer
local function timestamp()
    local result = redis.call('TIME')
    local seconds = tonumber(result[1]) + tonumber(result[2]) / 1000000
    return seconds
end

local function timer_zadd(keys, args)
...
local function timer_poll(keys, args)
    local ts = timestamp()
...
redis.register_function('timer_zadd', timer_zadd)
redis.register_function('timer_poll', timer_poll)
"""
...
class Timer:
    ...
    def new(self, key: str, data: str, interval: timedelta, *, loop=False):
        ...
        return self.redis.fcall('timer_zadd', 2, *keys_and_args)
    ...
    def poll(self, limit=100):
        ...
        res = self.redis.fcall('timer_poll', 2, *keys_and_args)

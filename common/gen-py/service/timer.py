#
# Autogenerated by Thrift Compiler (0.24.0)
#
# DO NOT EDIT UNLESS YOU ARE SURE THAT YOU KNOW WHAT YOU ARE DOING
#
#  options string: py
#

from thrift.Thrift import TType, TMessageType, TFrozenDict, TException, TApplicationException
from thrift.protocol.TProtocol import TProtocolException
from thrift.TRecursive import fix_spec
from uuid import UUID

import sys
import logging
from .ttypes import *
from thrift.Thrift import TProcessor
from thrift.transport import TTransport
all_structs = []


class Iface(object):
    def call_later(self, service, key, data, delay):
        """
        Parameters:
         - service
         - key
         - data
         - delay

        """
        pass

    def call_repeat(self, service, key, data, interval):
        """
        Parameters:
         - service
         - key
         - data
         - interval

        """
        pass

    def remove_timer(self, service, key):
        """
        Parameters:
         - service
         - key

        """
        pass

    def _migrate_timer(self, info_data):
        """
        Parameters:
         - info_data

        """
        pass

    def _delete_timer(self, service, key, uniq_id):
        """
        Parameters:
         - service
         - key
         - uniq_id

        """
        pass


class Client(Iface):
    def __init__(self, iprot, oprot=None):
        self._iprot = self._oprot = iprot
        if oprot is not None:
            self._oprot = oprot
        self._seqid = 0

    def call_later(self, service, key, data, delay):
        """
        Parameters:
         - service
         - key
         - data
         - delay

        """
        self.send_call_later(service, key, data, delay)
        self.recv_call_later()

    def send_call_later(self, service, key, data, delay):
        self._oprot.writeMessageBegin('call_later', TMessageType.CALL, self._seqid)
        args = call_later_args()
        args.service = service
        args.key = key
        args.data = data
        args.delay = delay
        args.write(self._oprot)
        self._oprot.writeMessageEnd()
        self._oprot.trans.flush()

    def recv_call_later(self):
        iprot = self._iprot
        (fname, mtype, rseqid) = iprot.readMessageBegin()
        if mtype == TMessageType.EXCEPTION:
            x = TApplicationException()
            x.read(iprot)
            iprot.readMessageEnd()
            raise x
        result = call_later_result()
        result.read(iprot)
        iprot.readMessageEnd()
        return

    def call_repeat(self, service, key, data, interval):
        """
        Parameters:
         - service
         - key
         - data
         - interval

        """
        self.send_call_repeat(service, key, data, interval)
        self.recv_call_repeat()

    def send_call_repeat(self, service, key, data, interval):
        self._oprot.writeMessageBegin('call_repeat', TMessageType.CALL, self._seqid)
        args = call_repeat_args()
        args.service = service
        args.key = key
        args.data = data
        args.interval = interval
        args.write(self._oprot)
        self._oprot.writeMessageEnd()
        self._oprot.trans.flush()

    def recv_call_repeat(self):
        iprot = self._iprot
        (fname, mtype, rseqid) = iprot.readMessageBegin()
        if mtype == TMessageType.EXCEPTION:
            x = TApplicationException()
            x.read(iprot)
            iprot.readMessageEnd()
            raise x
        result = call_repeat_result()
        result.read(iprot)
        iprot.readMessageEnd()
        return

    def remove_timer(self, service, key):
        """
        Parameters:
         - service
         - key

        """
        self.send_remove_timer(service, key)
        self.recv_remove_timer()

    def send_remove_timer(self, service, key):
        self._oprot.writeMessageBegin('remove_timer', TMessageType.CALL, self._seqid)
        args = remove_timer_args()
        args.service = service
        args.key = key
        args.write(self._oprot)
        self._oprot.writeMessageEnd()
        self._oprot.trans.flush()

    def recv_remove_timer(self):
        iprot = self._iprot
        (fname, mtype, rseqid) = iprot.readMessageBegin()
        if mtype == TMessageType.EXCEPTION:
            x = TApplicationException()
            x.read(iprot)
            iprot.readMessageEnd()
            raise x
        result = remove_timer_result()
        result.read(iprot)
        iprot.readMessageEnd()
        return

    def _migrate_timer(self, info_data):
        """
        Parameters:
         - info_data

        """
        self.send__migrate_timer(info_data)
        self.recv__migrate_timer()

    def send__migrate_timer(self, info_data):
        self._oprot.writeMessageBegin('_migrate_timer', TMessageType.CALL, self._seqid)
        args = _migrate_timer_args()
        args.info_data = info_data
        args.write(self._oprot)
        self._oprot.writeMessageEnd()
        self._oprot.trans.flush()

    def recv__migrate_timer(self):
        iprot = self._iprot
        (fname, mtype, rseqid) = iprot.readMessageBegin()
        if mtype == TMessageType.EXCEPTION:
            x = TApplicationException()
            x.read(iprot)
            iprot.readMessageEnd()
            raise x
        result = _migrate_timer_result()
        result.read(iprot)
        iprot.readMessageEnd()
        return

    def _delete_timer(self, service, key, uniq_id):
        """
        Parameters:
         - service
         - key
         - uniq_id

        """
        self.send__delete_timer(service, key, uniq_id)

    def send__delete_timer(self, service, key, uniq_id):
        self._oprot.writeMessageBegin('_delete_timer', TMessageType.ONEWAY, self._seqid)
        args = _delete_timer_args()
        args.service = service
        args.key = key
        args.uniq_id = uniq_id
        args.write(self._oprot)
        self._oprot.writeMessageEnd()
        self._oprot.trans.flush()


class Processor(Iface, TProcessor):
    def __init__(self, handler):
        self._handler = handler
        self._processMap = {}
        self._processMap["call_later"] = Processor.process_call_later
        self._processMap["call_repeat"] = Processor.process_call_repeat
        self._processMap["remove_timer"] = Processor.process_remove_timer
        self._processMap["_migrate_timer"] = Processor.process__migrate_timer
        self._processMap["_delete_timer"] = Processor.process__delete_timer
        self._on_message_begin = None

    def on_message_begin(self, func):
        self._on_message_begin = func

    def process(self, iprot, oprot):
        (name, type, seqid) = iprot.readMessageBegin()
        if self._on_message_begin:
            self._on_message_begin(name, type, seqid)
        if name not in self._processMap:
            iprot.skip(TType.STRUCT)
            iprot.readMessageEnd()
            x = TApplicationException(TApplicationException.UNKNOWN_METHOD, 'Unknown function %s' % (name))
            oprot.writeMessageBegin(name, TMessageType.EXCEPTION, seqid)
            x.write(oprot)
            oprot.writeMessageEnd()
            oprot.trans.flush()
            return
        else:
            self._processMap[name](self, seqid, iprot, oprot)
        return True

    def process_call_later(self, seqid, iprot, oprot):
        args = call_later_args()
        args.read(iprot)
        iprot.readMessageEnd()
        result = call_later_result()
        try:
            self._handler.call_later(args.service, args.key, args.data, args.delay)
            msg_type = TMessageType.REPLY
        except TTransport.TTransportException:
            raise
        except TApplicationException as ex:
            logging.exception('TApplication exception in handler')
            msg_type = TMessageType.EXCEPTION
            result = ex
        except Exception:
            logging.exception('Unexpected exception in handler')
            msg_type = TMessageType.EXCEPTION
            result = TApplicationException(TApplicationException.INTERNAL_ERROR, 'Internal error')
        oprot.writeMessageBegin("call_later", msg_type, seqid)
        result.write(oprot)
        oprot.writeMessageEnd()
        oprot.trans.flush()

    def process_call_repeat(self, seqid, iprot, oprot):
        args = call_repeat_args()
        args.read(iprot)
        iprot.readMessageEnd()
        result = call_repeat_result()
        try:
            self._handler.call_repeat(args.service, args.key, args.data, args.interval)
            msg_type = TMessageType.REPLY
        except TTransport.TTransportException:
            raise
        except TApplicationException as ex:
            logging.exception('TApplication exception in handler')
            msg_type = TMessageType.EXCEPTION
            result = ex
        except Exception:
            logging.exception('Unexpected exception in handler')
            msg_type = TMessageType.EXCEPTION
            result = TApplicationException(TApplicationException.INTERNAL_ERROR, 'Internal error')
        oprot.writeMessageBegin("call_repeat", msg_type, seqid)
        result.write(oprot)
        oprot.writeMessageEnd()
        oprot.trans.flush()

    def process_remove_timer(self, seqid, iprot, oprot):
        args = remove_timer_args()
        args.read(iprot)
        iprot.readMessageEnd()
        result = remove_timer_result()
        try:
            self._handler.remove_timer(args.service, args.key)
            msg_type = TMessageType.REPLY
        except TTransport.TTransportException:
            raise
        except TApplicationException as ex:
            logging.exception('TApplication exception in handler')
            msg_type = TMessageType.EXCEPTION
            result = ex
        except Exception:
            logging.exception('Unexpected exception in handler')
            msg_type = TMessageType.EXCEPTION
            result = TApplicationException(TApplicationException.INTERNAL_ERROR, 'Internal error')
        oprot.writeMessageBegin("remove_timer", msg_type, seqid)
        result.write(oprot)
        oprot.writeMessageEnd()
        oprot.trans.flush()

    def process__migrate_timer(self, seqid, iprot, oprot):
        args = _migrate_timer_args()
        args.read(iprot)
        iprot.readMessageEnd()
        result = _migrate_timer_result()
        try:
            self._handler._migrate_timer(args.info_data)
            msg_type = TMessageType.REPLY
        except TTransport.TTransportException:
            raise
        except TApplicationException as ex:
            logging.exception('TApplication exception in handler')
            msg_type = TMessageType.EXCEPTION
            result = ex
        except Exception:
            logging.exception('Unexpected exception in handler')
            msg_type = TMessageType.EXCEPTION
            result = TApplicationException(TApplicationException.INTERNAL_ERROR, 'Internal error')
        oprot.writeMessageBegin("_migrate_timer", msg_type, seqid)
        result.write(oprot)
        oprot.writeMessageEnd()
        oprot.trans.flush()

    def process__delete_timer(self, seqid, iprot, oprot):
        args = _delete_timer_args()
        args.read(iprot)
        iprot.readMessageEnd()
        try:
            self._handler._delete_timer(args.service, args.key, args.uniq_id)
        except TTransport.TTransportException:
            raise
        except Exception:
            logging.exception('Exception in oneway handler')

# HELPER FUNCTIONS AND STRUCTURES


class call_later_args(object):
    """
    Attributes:
     - service
     - key
     - data
     - delay

    """
    thrift_spec = None


    def __init__(self, service = None, key = None, data = None, delay = None,):
        self.service = service
        self.key = key
        self.data = data
        self.delay = delay

    def read(self, iprot):
        if iprot._fast_decode is not None and isinstance(iprot.trans, TTransport.CReadableTransport) and self.thrift_spec is not None:
            iprot._fast_decode(self, iprot, [self.__class__, self.thrift_spec])
            return
        iprot.increment_recursion_depth()
        try:
            iprot.readStructBegin()
            while True:
                (fname, ftype, fid) = iprot.readFieldBegin()
                if ftype == TType.STOP:
                    break
                if fid == 1:
                    if ftype == TType.STRING:
                        self.service = iprot.readString().decode('utf-8', errors='replace') if sys.version_info[0] == 2 else iprot.readString()
                    else:
                        iprot.skip(ftype)
                elif fid == 2:
                    if ftype == TType.STRING:
                        self.key = iprot.readString().decode('utf-8', errors='replace') if sys.version_info[0] == 2 else iprot.readString()
                    else:
                        iprot.skip(ftype)
                elif fid == 3:
                    if ftype == TType.STRING:
                        self.data = iprot.readString().decode('utf-8', errors='replace') if sys.version_info[0] == 2 else iprot.readString()
                    else:
                        iprot.skip(ftype)
                elif fid == 4:
                    if ftype == TType.DOUBLE:
                        self.delay = iprot.readDouble()
                    else:
                        iprot.skip(ftype)
                else:
                    iprot.skip(ftype)
                iprot.readFieldEnd()
            iprot.readStructEnd()
        finally:
            iprot.decrement_recursion_depth()

    def write(self, oprot):
        self.validate()
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, [self.__class__, self.thrift_spec]))
            return
        oprot.increment_recursion_depth()
        try:
            oprot.writeStructBegin('call_later_args')
            if self.service is not None:
                oprot.writeFieldBegin('service', TType.STRING, 1)
                oprot.writeString(self.service.encode('utf-8') if sys.version_info[0] == 2 else self.service)
                oprot.writeFieldEnd()
            if self.key is not None:
                oprot.writeFieldBegin('key', TType.STRING, 2)
                oprot.writeString(self.key.encode('utf-8') if sys.version_info[0] == 2 else self.key)
                oprot.writeFieldEnd()
            if self.data is not None:
                oprot.writeFieldBegin('data', TType.STRING, 3)
                oprot.writeString(self.data.encode('utf-8') if sys.version_info[0] == 2 else self.data)
                oprot.writeFieldEnd()
            if self.delay is not None:
                oprot.writeFieldBegin('delay', TType.DOUBLE, 4)
                oprot.writeDouble(self.delay)
                oprot.writeFieldEnd()
            oprot.writeFieldStop()
            oprot.writeStructEnd()
        finally:
            oprot.decrement_recursion_depth()

    def validate(self):
        return

    def __repr__(self):
        L = ['%s=%r' % (key, value)
             for key, value in self.__dict__.items()]
        return '%s(%s)' % (self.__class__.__name__, ', '.join(L))

    def __eq__(self, other):
        return isinstance(other, self.__class__) and self.__dict__ == other.__dict__

    def __ne__(self, other):
        return not (self == other)
all_structs.append(call_later_args)
call_later_args.thrift_spec = (
    None,  # 0
    (1, TType.STRING, 'service', 'UTF8', None, ),  # 1
    (2, TType.STRING, 'key', 'UTF8', None, ),  # 2
    (3, TType.STRING, 'data', 'UTF8', None, ),  # 3
    (4, TType.DOUBLE, 'delay', None, None, ),  # 4
)


class call_later_result(object):
    thrift_spec = None


    def read(self, iprot):
        if iprot._fast_decode is not None and isinstance(iprot.trans, TTransport.CReadableTransport) and self.thrift_spec is not None:
            iprot._fast_decode(self, iprot, [self.__class__, self.thrift_spec])
            return
        iprot.increment_recursion_depth()
        try:
            iprot.readStructBegin()
            while True:
                (fname, ftype, fid) = iprot.readFieldBegin()
                if ftype == TType.STOP:
                    break
                else:
                    iprot.skip(ftype)
                iprot.readFieldEnd()
            iprot.readStructEnd()
        finally:
            iprot.decrement_recursion_depth()

    def write(self, oprot):
        self.validate()
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, [self.__class__, self.thrift_spec]))
            return
        oprot.increment_recursion_depth()
        try:
            oprot.writeStructBegin('call_later_result')
            oprot.writeFieldStop()
            oprot.writeStructEnd()
        finally:
            oprot.decrement_recursion_depth()

    def validate(self):
        return

    def __repr__(self):
        L = ['%s=%r' % (key, value)
             for key, value in self.__dict__.items()]
        return '%s(%s)' % (self.__class__.__name__, ', '.join(L))

    def __eq__(self, other):
        return isinstance(other, self.__class__) and self.__dict__ == other.__dict__

    def __ne__(self, other):
        return not (self == other)
all_structs.append(call_later_result)
call_later_result.thrift_spec = (
)


class call_repeat_args(object):
    """
    Attributes:
     - service
     - key
     - data
     - interval

    """
    thrift_spec = None


    def __init__(self, service = None, key = None, data = None, interval = None,):
        self.service = service
        self.key = key
        self.data = data
        self.interval = interval

    def read(self, iprot):
        if iprot._fast_decode is not None and isinstance(iprot.trans, TTransport.CReadableTransport) and self.thrift_spec is not None:
            iprot._fast_decode(self, iprot, [self.__class__, self.thrift_spec])
            return
        iprot.increment_recursion_depth()
        try:
            iprot.readStructBegin()
            while True:
                (fname, ftype, fid) = iprot.readFieldBegin()
                if ftype == TType.STOP:
                    break
                if fid == 1:
                    if ftype == TType.STRING:
                        self.service = iprot.readString().decode('utf-8', errors='replace') if sys.version_info[0] == 2 else iprot.readString()
                    else:
                        iprot.skip(ftype)
                elif fid == 2:
                    if ftype == TType.STRING:
                        self.key = iprot.readString().decode('utf-8', errors='replace') if sys.version_info[0] == 2 else iprot.readString()
                    else:
                        iprot.skip(ftype)
                elif fid == 3:
                    if ftype == TType.STRING:
                        self.data = iprot.readString().decode('utf-8', errors='replace') if sys.version_info[0] == 2 else iprot.readString()
                    else:
                        iprot.skip(ftype)
                elif fid == 4:
                    if ftype == TType.DOUBLE:
                        self.interval = iprot.readDouble()
                    else:
                        iprot.skip(ftype)
                else:
                    iprot.skip(ftype)
                iprot.readFieldEnd()
            iprot.readStructEnd()
        finally:
            iprot.decrement_recursion_depth()

    def write(self, oprot):
        self.validate()
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, [self.__class__, self.thrift_spec]))
            return
        oprot.increment_recursion_depth()
        try:
            oprot.writeStructBegin('call_repeat_args')
            if self.service is not None:
                oprot.writeFieldBegin('service', TType.STRING, 1)
                oprot.writeString(self.service.encode('utf-8') if sys.version_info[0] == 2 else self.service)
                oprot.writeFieldEnd()
            if self.key is not None:
                oprot.writeFieldBegin('key', TType.STRING, 2)
                oprot.writeString(self.key.encode('utf-8') if sys.version_info[0] == 2 else self.key)
                oprot.writeFieldEnd()
            if self.data is not None:
                oprot.writeFieldBegin('data', TType.STRING, 3)
                oprot.writeString(self.data.encode('utf-8') if sys.version_info[0] == 2 else self.data)
                oprot.writeFieldEnd()
            if self.interval is not None:
                oprot.writeFieldBegin('interval', TType.DOUBLE, 4)
                oprot.writeDouble(self.interval)
                oprot.writeFieldEnd()
            oprot.writeFieldStop()
            oprot.writeStructEnd()
        finally:
            oprot.decrement_recursion_depth()

    def validate(self):
        return

    def __repr__(self):
        L = ['%s=%r' % (key, value)
             for key, value in self.__dict__.items()]
        return '%s(%s)' % (self.__class__.__name__, ', '.join(L))

    def __eq__(self, other):
        return isinstance(other, self.__class__) and self.__dict__ == other.__dict__

    def __ne__(self, other):
        return not (self == other)
all_structs.append(call_repeat_args)
call_repeat_args.thrift_spec = (
    None,  # 0
    (1, TType.STRING, 'service', 'UTF8', None, ),  # 1
    (2, TType.STRING, 'key', 'UTF8', None, ),  # 2
    (3, TType.STRING, 'data', 'UTF8', None, ),  # 3
    (4, TType.DOUBLE, 'interval', None, None, ),  # 4
)


class call_repeat_result(object):
    thrift_spec = None


    def read(self, iprot):
        if iprot._fast_decode is not None and isinstance(iprot.trans, TTransport.CReadableTransport) and self.thrift_spec is not None:
            iprot._fast_decode(self, iprot, [self.__class__, self.thrift_spec])
            return
        iprot.increment_recursion_depth()
        try:
            iprot.readStructBegin()
            while True:
                (fname, ftype, fid) = iprot.readFieldBegin()
                if ftype == TType.STOP:
                    break
                else:
                    iprot.skip(ftype)
                iprot.readFieldEnd()
            iprot.readStructEnd()
        finally:
            iprot.decrement_recursion_depth()

    def write(self, oprot):
        self.validate()
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, [self.__class__, self.thrift_spec]))
            return
        oprot.increment_recursion_depth()
        try:
            oprot.writeStructBegin('call_repeat_result')
            oprot.writeFieldStop()
            oprot.writeStructEnd()
        finally:
            oprot.decrement_recursion_depth()

    def validate(self):
        return

    def __repr__(self):
        L = ['%s=%r' % (key, value)
             for key, value in self.__dict__.items()]
        return '%s(%s)' % (self.__class__.__name__, ', '.join(L))

    def __eq__(self, other):
        return isinstance(other, self.__class__) and self.__dict__ == other.__dict__

    def __ne__(self, other):
        return not (self == other)
all_structs.append(call_repeat_result)
call_repeat_result.thrift_spec = (
)


class remove_timer_args(object):
    """
    Attributes:
     - service
     - key

    """
    thrift_spec = None


    def __init__(self, service = None, key = None,):
        self.service = service
        self.key = key

    def read(self, iprot):
        if iprot._fast_decode is not None and isinstance(iprot.trans, TTransport.CReadableTransport) and self.thrift_spec is not None:
            iprot._fast_decode(self, iprot, [self.__class__, self.thrift_spec])
            return
        iprot.increment_recursion_depth()
        try:
            iprot.readStructBegin()
            while True:
                (fname, ftype, fid) = iprot.readFieldBegin()
                if ftype == TType.STOP:
                    break
                if fid == 1:
                    if ftype == TType.STRING:
                        self.service = iprot.readString().decode('utf-8', errors='replace') if sys.version_info[0] == 2 else iprot.readString()
                    else:
                        iprot.skip(ftype)
                elif fid == 2:
                    if ftype == TType.STRING:
                        self.key = iprot.readString().decode('utf-8', errors='replace') if sys.version_info[0] == 2 else iprot.readString()
                    else:
                        iprot.skip(ftype)
                else:
                    iprot.skip(ftype)
                iprot.readFieldEnd()
            iprot.readStructEnd()
        finally:
            iprot.decrement_recursion_depth()

    def write(self, oprot):
        self.validate()
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, [self.__class__, self.thrift_spec]))
            return
        oprot.increment_recursion_depth()
        try:
            oprot.writeStructBegin('remove_timer_args')
            if self.service is not None:
                oprot.writeFieldBegin('service', TType.STRING, 1)
                oprot.writeString(self.service.encode('utf-8') if sys.version_info[0] == 2 else self.service)
                oprot.writeFieldEnd()
            if self.key is not None:
                oprot.writeFieldBegin('key', TType.STRING, 2)
                oprot.writeString(self.key.encode('utf-8') if sys.version_info[0] == 2 else self.key)
                oprot.writeFieldEnd()
            oprot.writeFieldStop()
            oprot.writeStructEnd()
        finally:
            oprot.decrement_recursion_depth()

    def validate(self):
        return

    def __repr__(self):
        L = ['%s=%r' % (key, value)
             for key, value in self.__dict__.items()]
        return '%s(%s)' % (self.__class__.__name__, ', '.join(L))

    def __eq__(self, other):
        return isinstance(other, self.__class__) and self.__dict__ == other.__dict__

    def __ne__(self, other):
        return not (self == other)
all_structs.append(remove_timer_args)
remove_timer_args.thrift_spec = (
    None,  # 0
    (1, TType.STRING, 'service', 'UTF8', None, ),  # 1
    (2, TType.STRING, 'key', 'UTF8', None, ),  # 2
)


class remove_timer_result(object):
    thrift_spec = None


    def read(self, iprot):
        if iprot._fast_decode is not None and isinstance(iprot.trans, TTransport.CReadableTransport) and self.thrift_spec is not None:
            iprot._fast_decode(self, iprot, [self.__class__, self.thrift_spec])
            return
        iprot.increment_recursion_depth()
        try:
            iprot.readStructBegin()
            while True:
                (fname, ftype, fid) = iprot.readFieldBegin()
                if ftype == TType.STOP:
                    break
                else:
                    iprot.skip(ftype)
                iprot.readFieldEnd()
            iprot.readStructEnd()
        finally:
            iprot.decrement_recursion_depth()

    def write(self, oprot):
        self.validate()
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, [self.__class__, self.thrift_spec]))
            return
        oprot.increment_recursion_depth()
        try:
            oprot.writeStructBegin('remove_timer_result')
            oprot.writeFieldStop()
            oprot.writeStructEnd()
        finally:
            oprot.decrement_recursion_depth()

    def validate(self):
        return

    def __repr__(self):
        L = ['%s=%r' % (key, value)
             for key, value in self.__dict__.items()]
        return '%s(%s)' % (self.__class__.__name__, ', '.join(L))

    def __eq__(self, other):
        return isinstance(other, self.__class__) and self.__dict__ == other.__dict__

    def __ne__(self, other):
        return not (self == other)
all_structs.append(remove_timer_result)
remove_timer_result.thrift_spec = (
)


class _migrate_timer_args(object):
    """
    Attributes:
     - info_data

    """
    thrift_spec = None


    def __init__(self, info_data = None,):
        self.info_data = info_data

    def read(self, iprot):
        if iprot._fast_decode is not None and isinstance(iprot.trans, TTransport.CReadableTransport) and self.thrift_spec is not None:
            iprot._fast_decode(self, iprot, [self.__class__, self.thrift_spec])
            return
        iprot.increment_recursion_depth()
        try:
            iprot.readStructBegin()
            while True:
                (fname, ftype, fid) = iprot.readFieldBegin()
                if ftype == TType.STOP:
                    break
                if fid == 1:
                    if ftype == TType.STRING:
                        self.info_data = iprot.readString().decode('utf-8', errors='replace') if sys.version_info[0] == 2 else iprot.readString()
                    else:
                        iprot.skip(ftype)
                else:
                    iprot.skip(ftype)
                iprot.readFieldEnd()
            iprot.readStructEnd()
        finally:
            iprot.decrement_recursion_depth()

    def write(self, oprot):
        self.validate()
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, [self.__class__, self.thrift_spec]))
            return
        oprot.increment_recursion_depth()
        try:
            oprot.writeStructBegin('_migrate_timer_args')
            if self.info_data is not None:
                oprot.writeFieldBegin('info_data', TType.STRING, 1)
                oprot.writeString(self.info_data.encode('utf-8') if sys.version_info[0] == 2 else self.info_data)
                oprot.writeFieldEnd()
            oprot.writeFieldStop()
            oprot.writeStructEnd()
        finally:
            oprot.decrement_recursion_depth()

    def validate(self):
        return

    def __repr__(self):
        L = ['%s=%r' % (key, value)
             for key, value in self.__dict__.items()]
        return '%s(%s)' % (self.__class__.__name__, ', '.join(L))

    def __eq__(self, other):
        return isinstance(other, self.__class__) and self.__dict__ == other.__dict__

    def __ne__(self, other):
        return not (self == other)
all_structs.append(_migrate_timer_args)
_migrate_timer_args.thrift_spec = (
    None,  # 0
    (1, TType.STRING, 'info_data', 'UTF8', None, ),  # 1
)


class _migrate_timer_result(object):
    thrift_spec = None


    def read(self, iprot):
        if iprot._fast_decode is not None and isinstance(iprot.trans, TTransport.CReadableTransport) and self.thrift_spec is not None:
            iprot._fast_decode(self, iprot, [self.__class__, self.thrift_spec])
            return
        iprot.increment_recursion_depth()
        try:
            iprot.readStructBegin()
            while True:
                (fname, ftype, fid) = iprot.readFieldBegin()
                if ftype == TType.STOP:
                    break
                else:
                    iprot.skip(ftype)
                iprot.readFieldEnd()
            iprot.readStructEnd()
        finally:
            iprot.decrement_recursion_depth()

    def write(self, oprot):
        self.validate()
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, [self.__class__, self.thrift_spec]))
            return
        oprot.increment_recursion_depth()
        try:
            oprot.writeStructBegin('_migrate_timer_result')
            oprot.writeFieldStop()
            oprot.writeStructEnd()
        finally:
            oprot.decrement_recursion_depth()

    def validate(self):
        return

    def __repr__(self):
        L = ['%s=%r' % (key, value)
             for key, value in self.__dict__.items()]
        return '%s(%s)' % (self.__class__.__name__, ', '.join(L))

    def __eq__(self, other):
        return isinstance(other, self.__class__) and self.__dict__ == other.__dict__

    def __ne__(self, other):
        return not (self == other)
all_structs.append(_migrate_timer_result)
_migrate_timer_result.thrift_spec = (
)


class _delete_timer_args(object):
    """
    Attributes:
     - service
     - key
     - uniq_id

    """
    thrift_spec = None


    def __init__(self, service = None, key = None, uniq_id = None,):
        self.service = service
        self.key = key
        self.uniq_id = uniq_id

    def read(self, iprot):
        if iprot._fast_decode is not None and isinstance(iprot.trans, TTransport.CReadableTransport) and self.thrift_spec is not None:
            iprot._fast_decode(self, iprot, [self.__class__, self.thrift_spec])
            return
        iprot.increment_recursion_depth()
        try:
            iprot.readStructBegin()
            while True:
                (fname, ftype, fid) = iprot.readFieldBegin()
                if ftype == TType.STOP:
                    break
                if fid == 1:
                    if ftype == TType.STRING:
                        self.service = iprot.readString().decode('utf-8', errors='replace') if sys.version_info[0] == 2 else iprot.readString()
                    else:
                        iprot.skip(ftype)
                elif fid == 2:
                    if ftype == TType.STRING:
                        self.key = iprot.readString().decode('utf-8', errors='replace') if sys.version_info[0] == 2 else iprot.readString()
                    else:
                        iprot.skip(ftype)
                elif fid == 3:
                    if ftype == TType.STRING:
                        self.uniq_id = iprot.readString().decode('utf-8', errors='replace') if sys.version_info[0] == 2 else iprot.readString()
                    else:
                        iprot.skip(ftype)
                else:
                    iprot.skip(ftype)
                iprot.readFieldEnd()
            iprot.readStructEnd()
        finally:
            iprot.decrement_recursion_depth()

    def write(self, oprot):
        self.validate()
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, [self.__class__, self.thrift_spec]))
            return
        oprot.increment_recursion_depth()
        try:
            oprot.writeStructBegin('_delete_timer_args')
            if self.service is not None:
                oprot.writeFieldBegin('service', TType.STRING, 1)
                oprot.writeString(self.service.encode('utf-8') if sys.version_info[0] == 2 else self.service)
                oprot.writeFieldEnd()
            if self.key is not None:
                oprot.writeFieldBegin('key', TType.STRING, 2)
                oprot.writeString(self.key.encode('utf-8') if sys.version_info[0] == 2 else self.key)
                oprot.writeFieldEnd()
            if self.uniq_id is not None:
                oprot.writeFieldBegin('uniq_id', TType.STRING, 3)
                oprot.writeString(self.uniq_id.encode('utf-8') if sys.version_info[0] == 2 else self.uniq_id)
                oprot.writeFieldEnd()
            oprot.writeFieldStop()
            oprot.writeStructEnd()
        finally:
            oprot.decrement_recursion_depth()

    def validate(self):
        return

    def __repr__(self):
        L = ['%s=%r' % (key, value)
             for key, value in self.__dict__.items()]
        return '%s(%s)' % (self.__class__.__name__, ', '.join(L))

    def __eq__(self, other):
        return isinstance(other, self.__class__) and self.__dict__ == other.__dict__

    def __ne__(self, other):
        return not (self == other)
all_structs.append(_delete_timer_args)
_delete_timer_args.thrift_spec = (
    None,  # 0
    (1, TType.STRING, 'service', 'UTF8', None, ),  # 1
    (2, TType.STRING, 'key', 'UTF8', None, ),  # 2
    (3, TType.STRING, 'uniq_id', 'UTF8', None, ),  # 3
)
fix_spec(all_structs)
del all_structs
